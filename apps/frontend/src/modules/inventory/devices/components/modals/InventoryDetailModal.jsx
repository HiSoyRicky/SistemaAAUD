...
function InventoryDetailModal({ isOpen, onClose, item }) {
    ...
    const locationHistory = useMemo(() => {
        return historyRows.filter((row) => {
            const hasUserChange = row.previous_user !== row.new_user;
            const hasUbicationChange = row.previous_ubication !== row.new_ubication;
            const hasDepartmentChange = row.previous_department !== row.new_department;
                                    ...
                                    locationHistory.map((row) => (
                                        <tr key={row.id}>
                                            <td className="px-2 py-2 text-center border">
                                                {row.moved_at ? new Date(row.moved_at).toLocaleDateString('es-PA') : '-'}
                                            </td>
                                            <td className="px-2 py-2 text-center border">
