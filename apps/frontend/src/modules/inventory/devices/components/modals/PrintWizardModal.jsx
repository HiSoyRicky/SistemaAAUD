...
function canGoNext({ docType, step, localDevice }) {
  if (docType === 'delete') return true;
  if (step === 2) return !!localDevice?.role;
  if (step === 3) {
    return !!localDevice?.ubication_destino_id && !!localDevice?.department_destino_id;
  }

  return true;
}

function getNextStep({ docType, step }) {
  if (docType === 'delete') return 2;
...
export default function PrintWizardModal({
  ...
  const summary = useMemo(() => {
    ...
    return [
      ...
      {
        label: 'Marca / Modelo',
        value: `${localDevice.brand_name || '-'} ${localDevice.model_name || ''}`.trim(),
      },
  ...
  const next = () => {
    const canProceed = canGoNext({ docType, step, localDevice });

    if (!canProceed) return;

    setStep((currentStep) => getNextStep({ docType, step: currentStep }));
  };
  ...
  const getConfirmButtonLabel = () => {
    ...
    if (docType === 'transfer') return 'Imprimir traslado';

    return 'Imprimir descarte';
  };
        ...
        <div className="flex items-start justify-between gap-3 p-4 border-b md:p-5">
          <div>
            <h3 className="text-lg font-extrabold text-gray-900 md:text-xl">
              {docType === 'transfer'
                ? 'Asistente de impresión — Traslado'
                : 'Asistente de impresión — Descarte'}
            </h3>
                      ...
                      {docType === 'transfer' && (
                        <div className="p-3 mt-3 text-sm text-blue-900 border border-blue-100 rounded-xl bg-blue-50">
                          ...
                          <div className="mt-1 text-blue-800/90">
                            <span className="font-medium">Ubicación destino:</span>{' '}
                            {localDevice.ubication_destino_name || '-'}
                          </div>
                          <div className="mt-1 text-blue-800/90">
            ...
            <div className="flex items-center gap-2">
              {((docType === 'delete' && step === 1) || (docType === 'transfer' && step < 4)) && (
                <button
                  type="button"
                  onClick={next}
                  disabled={!canGoNext({ docType, step, localDevice }) || submitting}
...
function StepPill({ n, active, done, label }) {
  let stepClasses = 'bg-gray-200 text-gray-700';
  ...
  if (active) {
    stepClasses = 'bg-blue-600 text-white';
  }

  const labelClasses = active ? 'text-gray-900 font-semibold' : 'text-gray-600';
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={['text-xs md:text-sm', labelClasses].join(' ')}>{label}</span>
    </div>
  );
}
