  ...
  const handleMovement = async () => {
    ...
    try {
      ...
      const res = await Toners.addMovement({
        ...
        id_department: movementType === 'OUT' ? Number(selectedDepartment) : null,
        id_ubication: movementType === 'OUT' ? Number(selectedUbication) : null,
        reference: notes,
      ...
      addNotification(error.response?.data?.message || 'Error al registrar movimiento ❌', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative w-[620px] max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold">Movimiento de Tóner</h2>
              <p className="text-sm text-gray-500">
                {toner?.toner_model} • Stock actual:{' '}
                <span className="font-semibold text-gray-800">{toner?.stock}</span>
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-6">
          {/* Tipo de movimiento */}
          <div>
            <label htmlFor="movementType" className="block mb-2 text-sm font-medium text-gray-700">
              Tipo de Movimiento
            </label>
            ...
            <div className="grid grid-cols-3 gap-3">
              <button
                ...
                className={`flex items-center justify-center gap-2 py-2 rounded-xl border transition ${
                  movementType === 'IN'
          ...
          {isOut && (
            <div className="pt-2">
              <h3 className="mb-2 text-sm font-semibold text-gray-700">Destino del Tóner</h3>
              <UbiDepSelector
          ...
          {isOut && (
            <div>
              <label htmlFor="receiverName" className="block mb-2 text-sm font-medium">
